        ...
        <div key={step.id} className="flex items-center">
          <div className="flex items-center gap-3">
            <div className={`relative flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-all duration-300 ${
              currentStep > step.id
                ...
                : currentStep === step.id
                  ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 ring-4 ring-orange-500/10'
          ...
          {i < STEPS.length - 1 && (
            <div className={`h-0.5 w-8 md:w-16 mx-3 rounded-full transition-colors duration-300 ${
              currentStep > step.id ? 'bg-orange-500' : 'bg-slate-200 dark:bg-slate-800'
            }`} />
          )}
        </div>
      ))}
...
function StarterCard({ starter, onSelect }: { starter: StarterTemplate; onSelect: (s: StarterTemplate) => void }) {
  const categoryColor: Record<string, string> = {
    Marketing: 'bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20',
...
export default function TemplatesPage() {
          ...
          <div className="flex items-center gap-2">
            ...
            {step === 2 && (
              <Button
                ...
                className="bg-orange-600 hover:bg-orange-700 text-white font-semibold text-xs h-9 disabled:opacity-50"
              >
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                ...
                >
                  ...
                  <div className="rounded-xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900/20 p-5 space-y-4">
                    <div className="flex items-center gap-2 pb-2 border-b border-slate-100 dark:border-slate-800/60">
                      <AlignLeft className="size-4 text-orange-500" />
                    ...
                    <div className="space-y-3">
                      <Textarea
                        ...
                        rows={5}
                        className="resize-none text-xs border-slate-200 dark:border-slate-800 focus-visible:ring-orange-500/20"
                      />
                      ...
                      <div className="flex gap-2">
                        {([
                          ...
                          <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 rounded-lg border border-dashed border-slate-300 dark:border-slate-800 px-3 py-2 text-xs text-slate-500 dark:text-slate-400 hover:border-orange-500 hover:text-orange-600 dark:hover:text-orange-400 transition-colors"
                          >
                            ...
                            <div key={i} className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60 p-4 space-y-2.5 relative">
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                                  {btn.type === 'URL' ? <Link2 className="size-3.5 text-orange-500" /> : btn.type === 'PHONE_NUMBER' ? <Phone className="size-3.5 text-orange-500" /> : <MessageSquareReply className="size-3.5 text-orange-500" />}
                ...
                >
                  <div>
                    <h2 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                      <BrainCircuit className="size-5 text-orange-500" />
                      AI Policy Review
                    </h2>
                  ...
                  {aiChecking && (
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 py-16 gap-4">
                      <div className="relative">
                        <div className="h-14 w-14 rounded-full border-4 border-orange-500/20 border-t-emerald-500 animate-spin" />
                        <BrainCircuit className="absolute inset-0 m-auto size-6 text-orange-500 animate-pulse" />
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className={`rounded-xl border p-5 flex items-center gap-5 ${
                        ...
                        <div className={`relative flex h-16 w-16 shrink-0 items-center justify-center rounded-full border-4 ${
                          aiReview.passed ? 'border-orange-500 bg-orange-500/10' : 'border-red-500 bg-red-500/10'
                        }`}>
                          <span className={`text-xl font-black ${
                            aiReview.passed ? 'text-orange-600 dark:text-orange-400' : 'text-red-600 dark:text-red-400'
                          }`}>{aiReview.score}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <p className={`text-sm font-bold ${
                              aiReview.passed ? 'text-orange-700 dark:text-orange-400' : 'text-red-700 dark:text-red-400'
                          ...
                          <div className="mt-2.5 h-1.5 w-full rounded-full bg-slate-200 dark:bg-slate-800">
                            <div
                              className={`h-1.5 rounded-full transition-all duration-700 ${
                                aiReview.score >= 80 ? 'bg-orange-500' : aiReview.score >= 50 ? 'bg-amber-500' : 'bg-red-500'
                              }`}
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {!check.passed && check.suggestion && (
                                <p className="text-[11px] mt-1.5 flex items-start gap-1.5 text-slate-600 dark:text-slate-300 font-medium">
                                  <ArrowRight className="size-3 mt-0.5 shrink-0 text-orange-500" />
                                  {check.suggestion}
                                </p>
                              )}
                            </div>
              ...
              {step === 3 && (
                ...
                >
                  ...
                  {aiReview && (
                    <div className="flex items-start gap-3 rounded-xl bg-orange-50 dark:bg-orange-500/5 border border-orange-200 dark:border-orange-500/20 p-4">
                      <ShieldCheck className="size-4 shrink-0 text-orange-500 mt-0.5" />
                      <div className="space-y-0.5">
                        <p className="text-xs font-bold text-orange-800 dark:text-orange-400">AI Review Passed — Score {aiReview.score}/100</p>
                        <p className="text-[11px] text-orange-700/90 dark:text-orange-400/80 leading-relaxed">{aiReview.summary}</p>
                      </div>
                    </div>
                  )}
              ...
              {step === 4 && (
                ...
                >
                  ...
                  {/* AI Review badge */}
                  {aiReview && (
                    <div className="flex items-center gap-3 rounded-xl bg-orange-50 dark:bg-orange-500/5 border border-orange-200 dark:border-orange-500/20 p-4">
                      <ShieldCheck className="size-5 shrink-0 text-orange-500" />
                      <div>
                        <p className="text-xs font-bold text-orange-700 dark:text-orange-400">AI Review Passed &middot; Score {aiReview.score}/100</p>
                        <p className="text-[11px] text-orange-600/80 dark:text-orange-400/60">This template passed all Meta policy checks and is ready to submit.</p>
                      </div>
                    </div>
                  )}
                        ...
                        <div key={idx} className="flex items-start gap-3">
                          <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${
                            t.state === 'done'
                              ...
                              : t.state === 'active'
                                ? 'bg-orange-500/20 text-orange-500 border border-orange-500/40 animate-pulse'
                                : 'bg-slate-100 dark:bg-slate-800 text-slate-400 border border-slate-200 dark:border-slate-700'
                          }`}>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <LayoutTemplate className="size-6 text-orange-500" />
            Message Templates
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="h-9 gap-2 bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold shadow-md shadow-orange-600/10"
          >
      ...
      {!loading && templates.length > 0 && (
        <div className="flex flex-wrap gap-3">
          {[
            ...
            { label: 'Approved', value: approved, icon: CheckCircle2, color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-500/10' },
      ...
      <div className="space-y-3.5">
        <div className="flex items-center gap-2">
          <Sparkles className="size-4.5 text-orange-500 animate-pulse" />
          <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200">Starter Templates</h2>
          <Badge className="bg-orange-500/10 text-orange-600 dark:text-orange-400 border-0 text-[10px] font-bold">
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 py-16 px-4 text-center">
            {search ? (
              ...
              <>
                ...
                <Button
                  className="mt-4 bg-orange-600 hover:bg-orange-700 text-white h-9 text-xs font-semibold"
                  onClick={() => { setForm(emptyForm); setView('builder'); setStep(1); }}
                >
