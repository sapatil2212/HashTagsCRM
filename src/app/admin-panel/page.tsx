...
export default function AdminOverviewPage() {
  ...
  const STAT_CARDS = stats ? [
    { label: "Total Users",        value: String(stats.totalUsers),              sub: `${stats.confirmedUsers} confirmed`,    icon: Users,        accent: "text-orange-400 bg-orange-500/10 border-orange-500/20" },
    { label: "WA Connected",       value: String(stats.activeUsers),             sub: "active integrations",                  icon: CheckCircle,  accent: "text-orange-400 bg-orange-500/10 border-orange-500/20" },
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <p className="text-sm font-semibold text-white flex items-center gap-2">
                      <TrendingUp className="size-4 text-orange-400" /> User Signups
                    </p>
                    <p className="text-xs text-slate-500 mt-0.5">Last 12 months</p>
                  </div>
                  <span className="text-xs bg-orange-500/10 border border-orange-500/20 text-orange-400 px-2 py-0.5 rounded-full font-bold">
                ...
                <div className="flex items-end gap-1.5 h-28">
                  {signups.map((m, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <div
                        className={cn("w-full rounded-sm", i === signups.length - 1 ? "bg-orange-500" : "bg-slate-700")}
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900 p-5">
                <p className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                  <Activity className="size-4 text-orange-400" /> Platform Stats
                </p>
                <div className="space-y-4">
                  {[
                    { label: "Total Users",        value: stats?.totalUsers ?? 0,         color: "bg-orange-500" },
                    { label: "WA Connected",       value: stats?.activeUsers ?? 0,        color: "bg-orange-500" },
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
                <p className="text-sm font-semibold text-white flex items-center gap-2">
                  <Users className="size-4 text-orange-400" /> Recent Users
                </p>
                <Link href="/admin-panel/clients" className="text-xs text-orange-400 hover:text-orange-300 font-semibold transition-colors">
                  View all →
                </Link>
              </div>
                      ...
                      <tr key={c.id} className="border-b border-slate-800/50 last:border-0 hover:bg-slate-800/30 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2.5">
                            <div className="w-7 h-7 rounded-lg bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400 text-xs font-bold shrink-0">
