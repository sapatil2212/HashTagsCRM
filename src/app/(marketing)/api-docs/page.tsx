...
export default function APIDocsLanding() {
  ...
  const codes = {
    curl: `curl -X POST https://api.hashtagscrm.com/v1/messages \\
  -H "Authorization: Bearer hashtagscrm_sec_live_..." \\
  ...
  -d '{
    ...
    node: `const client = require('@hashtagscrm/sdk')('hashtagscrm_sec_live_...');
...
await client.messages.sendTemplate({
  ...
  bodyParameters: ['Acme Partner']
    ...
    python: `import hashtagscrm

client = hashtagscrm.Client(api_key='hashtagscrm_sec_live_...')
      ...
      <div className="relative">
        ...
        <div className="rounded-2xl border border-[var(--m-border-primary)] bg-[var(--m-bg-primary)] p-4 shadow-2xl relative z-10 flex flex-col h-[380px] justify-between">
              ...
              {(["curl", "node", "python"] as const).map((t) => (
                <button
                  ...
                  className={`text-[9px] font-bold px-2 py-0.5 rounded cursor-pointer uppercase ${
                    lang === t ? "bg-orange-500/10 border border-orange-500/30 text-orange-400" : "text-[var(--m-text-muted)]"
                  }`}
                >
                  {t}
                </button>
          ...
          <div className="flex items-center justify-between border-t border-[var(--m-border-primary)] pt-4 mt-3">
            ...
            <span className="text-[9px] bg-orange-500/10 border border-orange-500/20 text-orange-400 rounded px-1.5 py-0.5 font-bold">API ACTIVE</span>
          </div>
        </div>
      </div>
    </div>
