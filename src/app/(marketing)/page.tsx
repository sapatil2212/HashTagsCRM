    ...
    <div className="w-full flex flex-col overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto flex flex-col items-center text-center relative z-10 pt-4">
          ...
          >
            <MagneticButton>
              <Link
                href="/login"
                className="bg-[#FFA500] hover:bg-[#EA580C] text-white px-8 py-3 rounded-full text-xs font-bold transition-all shadow-[0_4px_20px_rgba(255,165,0,0.25)] flex items-center justify-center"
              >
                Get Started
              </Link>
            </MagneticButton>
      ...
      <section className="py-24 px-4 md:px-6 max-w-6xl mx-auto space-y-16 relative">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8">
          ...
          <motion.div
            ...
            className="absolute top-[20px] left-[20px] w-[calc(66.666%)] h-[1px] bg-orange-500/20 hidden md:block z-0 origin-left"
          />
          ...
          >
            <div className="w-10 h-10 rounded-lg border border-orange-500/30 bg-[var(--m-bg-primary)] flex items-center justify-center text-xs font-semibold text-orange-400 mb-6 shadow-md shadow-orange-500/5 group-hover:border-orange-500 group-hover:text-orange-300 transition-all duration-300">
              01
            </div>
      ...
      <section className="py-24 px-4 md:px-6 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative">
        <div className="space-y-6">
          <div className="inline-flex items-center gap-1.5 text-xs font-bold text-orange-400 uppercase tracking-wide">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            ...
            <div className="flex gap-2">
              <span className="text-orange-400 font-semibold font-mono">✓</span>
              <div>
                <h4 className="text-xs font-semibold text-[var(--m-text-secondary)] transition-colors duration-300">
        ...
        <div className="relative">
          <div className="absolute inset-0 bg-orange-500/5 blur-[90px] pointer-events-none" />
          <AIChatSimulation />
        </div>
      </section>
      ...
      <section className="py-24 px-4 md:px-6 border-y border-[var(--m-border-primary)] bg-[var(--m-bg-secondary)]/10 transition-colors duration-300 relative">
        <div className="max-w-6xl mx-auto space-y-12 relative z-10">
          <motion.div
            ...
            className="text-center max-w-3xl mx-auto space-y-4"
          >
            <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-orange-500/25 bg-orange-950/20 text-orange-400 text-[11px] font-semibold tracking-wide uppercase">
              ✨ Social Proof
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-semibold tracking-tight text-[var(--m-text-heading)] transition-colors duration-300">
      ...
      <section className="py-24 max-w-full overflow-hidden space-y-12 relative">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto px-4 md:px-6">
          <span className="text-xs font-bold uppercase tracking-widest text-orange-400">Success Stories</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {pricingCards.map((plan, idx) => (
            <SpotlightCard
              key={idx}
              interactive={true}
              className={plan.primary ? "border-orange-500/50 shadow-lg relative" : "border-[var(--m-border-primary)]/80"}
              glowColor={plan.primary ? "rgba(255, 165, 0, 0.15)" : "rgba(148, 163, 184, 0.08)"}
            >
              <div className="space-y-6 flex flex-col justify-between h-full">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {plan.primary && (
                      <span className="bg-orange-500 text-white text-[9px] font-bold uppercase px-2 py-0.5 rounded-full tracking-wider">
                        Recommended
                      </span>
                    )}
                  </div>
                  ...
                  {plan.renewal && (
                    <p className="text-[10px] text-orange-400 font-medium">{plan.renewal}</p>
                  )}
                ...
                <div className="space-y-4 pt-4">
                  <ul className="space-y-2 border-t border-[var(--m-border-primary)]/40 pt-4 text-xs text-[var(--m-text-tertiary)]">
                    {plan.features.slice(0, 4).map((f, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <Check className="size-3.5 text-orange-400 shrink-0" />
                        <span>{f}</span>
                      </li>
                    ))}
                  </ul>
                  <Link
                    ...
                    className={`w-full py-2.5 rounded-xl text-xs font-bold transition-all text-center flex items-center justify-center gap-1.5 ${
                      plan.primary
                        ? "bg-orange-500 text-white hover:bg-orange-400 shadow-[0_0_12px_rgba(255,165,0,0.2)]"
      ...
      <section className="py-24 px-4 md:px-6 border-t border-[var(--m-border-primary)] max-w-4xl mx-auto space-y-16">
        <div className="text-center space-y-4">
          <span className="text-xs font-bold uppercase tracking-widest text-orange-400">Common Questions</span>
          <h2 className="text-3xl sm:text-4xl font-semibold text-[var(--m-text-heading)] tracking-tight">
          ...
          {faqs.map((faq, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span>{faq.q}</span>
                  {isOpen ? <ChevronUp className="size-4 text-orange-400" /> : <ChevronDown className="size-4 text-[var(--m-text-muted)]" />}
                </button>
      ...
      <section className="py-24 px-4 md:px-6 relative text-center max-w-6xl mx-auto border-t border-[var(--m-border-primary)] transition-colors duration-300">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[70%] h-[350px] rounded-full bg-orange-500/5 blur-[120px] pointer-events-none" />
        ...
        <div className="space-y-6 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <MagneticButton>
              <Link
                href="/login"
                className="bg-orange-500 hover:bg-orange-400 text-white px-6 py-3 rounded-xl text-xs font-bold transition-all shadow-[0_0_16px_rgba(255,165,0,0.2)] hover:shadow-[0_0_24px_rgba(255,165,0,0.4)] flex items-center gap-1.5"
              >
