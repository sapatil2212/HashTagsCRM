...
const statusConfig: Record<string, { color: string; icon: React.ElementType; label: string }> = {
  ...
  Approved: { color: 'bg-orange-500/10 text-orange-400 border-orange-500/20', icon: CheckCircle2, label: 'Approved' },
...
export function TemplateCard({ template, onDelete, onSelect, isSelected }: TemplateCardProps) {
  ...
  return (
    ...
    >
      ...
      {status === 'Approved' && (
        <div className="mt-2.5 flex items-center gap-1.5 rounded-md bg-orange-500/5 border border-orange-500/20 px-2 py-1">
          <CheckCircle2 className="size-3 text-orange-400" />
          <p className="text-[10px] text-orange-400">Ready to send in broadcasts</p>
        </div>
      )}
    </motion.div>
  );
